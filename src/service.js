...
const userService = {
  getAllUsers: async (page = 1, count = 10) => {
    try {
      const response = await axiosInstance.get(`/users/getAllUsers`, { params: { page, count } });
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
    }
...
const addressService = {
  ...
  getMandals: async (cityId) => {
    try {
      const response = await axiosInstance.get('/address/getMandals', { params: { cityId } });
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
    }
  ...
  deleteCity: async (cityId) => {
    try {
      const response = await axiosInstance.delete(`/address/deleteCity`, { data: { id: cityId } });
      return response.data;
    } catch (error) {
  ...
  uploadExcelData: async (excelData) => {
    try {
      const response = await axiosInstance.post(`/address/add_address_from_excel`, excelData, {
        headers: { 'Content-Type': 'application/json' },
      });
      return response.data;
    } catch (error) {
...
export { authService, userService, addressService, coordinatorService };
